...
import (
	"bytes"
	"encoding/json"
	keys "fluentqa-gochat/server/keys"
	"fmt"
...
func main() {
	...
	router.POST("/api_key/submit", func(c *gin.Context) {
		var api_key keys.APIKeySubmission
		c.BindJSON(&api_key)
		if api_key.APIKey == "" {
			c.JSON(400, gin.H{
		...
		creditSummary, err := keys.GetTotalCredits(api_key.APIKey)
		if err != nil {
			c.JSON(500, gin.H{
				"error": err.Error(),
			})
	...
	router.GET("/api_key/status", func(c *gin.Context) {
		...
		// Get credit summary
		creditSummary, err := keys.GetTotalCredits(key)
		if err != nil {
			c.JSON(500, gin.H{
				"error": err.Error(),
			})
...
func proxy(c *gin.Context) {
	...
	if response.StatusCode == 429 {
		// Check HardLimitUsd
		creditSummary, err := keys.GetTotalCredits(random_key)
		if err != nil {
			c.JSON(500, gin.H{
				"error": err.Error(),
			})
